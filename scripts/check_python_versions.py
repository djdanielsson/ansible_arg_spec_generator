...
def get_supported_python_versions() -> List[str]:
    ...
    """
    print("🔍 Fetching Python versions manifest...", file=sys.stderr)
    try:
        response = requests.get(VERSIONS_MANIFEST_URL, timeout=30)
        response.raise_for_status()
        releases = response.json()
    except requests.RequestException as e:
        print(f"❌ Could not fetch version manifest: {e}", file=sys.stderr)
        sys.exit(1)

    supported_versions: Set[str] = set()
    print(
        f"📋 Found {len(releases)} releases. Filtering for stable versions...",
        file=sys.stderr,
    )
    ...
    print(f"🎯 Supported stable versions: {sorted_versions}", file=sys.stderr)
...
def update_pyproject_toml(target_versions: List[str]) -> bool:
    ...
    if not classifier_match:
        print(
            "❌ Could not find 'classifiers' array in pyproject.toml.", file=sys.stderr
        )
        return False
...
def main() -> int:
    ...
    if args.format:
        if not supported_versions:
            print("❌ No supported Python versions found.", file=sys.stderr)
            return 1
        if args.format == "json":
    ...
    print("❗️ `pyproject.toml` needs updating.")
    added = sorted(list(supported_set - current_versions))
    removed = sorted(list(current_versions - supported_set))
    if added:
        print(f"   ➕ Versions to add: {added}")
    if removed:
        print(f"   ➖ Versions to remove: {removed}")

    # ✅ FIX: Use underscore instead of hyphen
    if args.check_only:
        print("\nℹ️ Exiting with status 1 due to --check-only flag.")
        return 1
